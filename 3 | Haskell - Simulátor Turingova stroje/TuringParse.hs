...
tmParser = TM <$> stateListP <* newline
              <*> alphabetP  <* newline
              <*> stateP     <* newline
              <*> stateP     <* newline
              <*> transitionsP
...
transP = Trans <$> stateP <* comma <*> symbP <* comma <*> stateP <* comma <*> actP
  where actP  =  ALeft <$ char '<'  <|>  ARight <$ char '>'  <|>  AWrite <$> symbP
...
validate tm@TM{..} = if allOK then Right tm else Left "invalid TM"
  where
